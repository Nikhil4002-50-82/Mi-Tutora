...
// Custom ID Generator (Matching the frontend logic)
...
async function migrateCollection(collectionName, prefix, mappingDict, addAuthUid = false) {
  ...
  for (const doc of snapshot.docs) {
    ...
    // Skip if it already starts with the prefix
    if (oldId.startsWith(prefix)) {
      console.log(`  - Skipping ${oldId} (Already migrated)`);
    ...
    if (addAuthUid) {
      newData.authUid = oldId; // The old ID for parents and tutors was the auth UID
    }
...
async function runMigration() {
  try {
    ...
    for (const doc of groupsSnap.docs) {
      if (!doc.id.startsWith('MTG')) continue;
      const data = doc.data();
      let updated = false;
      const updates = {};
    ...
    for (const doc of studentsSnap.docs) {
      if (!doc.id.startsWith('MTS')) continue;
      const data = doc.data();
      let updated = false;
      const updates = {};
    ...
    for (const doc of reqSnap.docs) {
      if (!doc.id.startsWith('REQ')) continue;
      const data = doc.data();
      let updated = false;
      const updates = {};

...
// Custom ID Generator (Matching the V1 logic: 6 random characters)
...
async function migrateCollection(collectionName, prefix, mappingDict, addAuthUid = false) {
  ...
  for (const doc of snapshot.docs) {
    ...
    // Only migrate if it has a hyphen (V2) or is entirely the wrong format
    // A correct V1 ID is exactly 9 chars without hyphen.
    if (!oldId.includes('-') && oldId.length === 9) {
      console.log(`  - Skipping ${oldId} (Already V1 format)`);
      mappingDict[oldId] = oldId;
      continue;
    }

    const data = doc.data();
    const newId = generateCustomId(prefix);
    ...
    // Keep authUid if it already exists
    if (addAuthUid && !newData.authUid && oldId.includes('-')) {
       // if for some reason it was missing, we could parse it, but it should be there.
    }
...
async function runMigration() {
  try {
    console.log("🚀 Starting Database ID Migration V3 (Reverting to V1)...");
    ...
    for (const doc of groupsSnap.docs) {
      if (doc.id.includes('-') || doc.id.length !== 9) continue; // Only process correctly formatted ones now
      const data = doc.data();
      let updated = false;
      const updates = {};
    ...
    for (const doc of studentsSnap.docs) {
      if (doc.id.includes('-') || doc.id.length !== 9) continue;
      const data = doc.data();
      let updated = false;
      const updates = {};
    ...
    console.log("\n🎉 Migration V3 (Revert to V1) completed successfully!");
    process.exit(0);
  } catch (error) {
